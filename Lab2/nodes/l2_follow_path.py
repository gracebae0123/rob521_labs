...
TRANS_VEL_OPTS = [0, 0.001, 0.01, 0.025, 0.13]  # m/s, max of real robot is .26
ROT_VEL_OPTS = np.linspace(-1.82, 1.82, 32)  # rad/s, max of real robot is 1.82
...
PATH_NAME = 'shortest_path_RRT_star_best_myhal.npy'  # saved path from l2_planning.py, should be in the same directory as this file
...
class PathFollower():
    def __init__(self):
        ...
        # build collision map (make walls thicker using the radius of the robot)
        coli_win = int(COLLISION_RADIUS / self.map_resolution)
        self.coli_map = self.map_np.copy()
        for i in range(len(self.coli_map)):
            for j in range(len(self.coli_map[i])):
                if self.map_np[i, j]:
                    lb = max(0, j - coli_win)
                    rb = min(map.info.width - 1, j + coli_win + 1)
                    ub = max(0, i - coli_win)
                    db = min(map.info.height - 1, i + coli_win + 1)
                    self.coli_map[ub:db, lb:rb] = True
    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            # print("Propogate the trajectory forward, storing the resulting points in local_paths!")
            cur_ang = -float(self.pose_in_map_np[..., 2])
            for t in range(1, self.horizon_timesteps + 1):

                # trans vel [0], rot vel [1]
                trans_vel = self.all_opts_scaled[:, 0]
                rot_vel = self.all_opts_scaled[:, 1]
                pos_t = np.asarray([(trans_vel / rot_vel) * np.sin(rot_vel * t),
                                    (trans_vel / rot_vel) * (1 - np.cos(rot_vel * t)),
                                    rot_vel * t]) # shape (3, self.num_opts)
                rot_mat = np.asarray([[np.cos(cur_ang), np.sin(cur_ang)],
                                     [-np.sin(cur_ang), np.cos(cur_ang)]])
                pos_t[:2, ...] = np.dot(rot_mat, pos_t[:2, ...]).copy()
                pos_t = pos_t.T

                pos_t = pos_t + local_paths[0]
                local_paths[t] = pos_t.copy()
            ...
            local_paths_pixels = local_paths_pixels.astype(np.int32)
            valid_opts = range(self.num_opts)
            local_paths_lowest_collision_dist = np.ones(self.num_opts) * 50

            # print("Check the points in local_path_pixels for collisions")
            valid_opts, final_cost = [], []
            for opt in range(local_paths_pixels.shape[1]):
                collisions = self.coli_map [local_paths_pixels[:, opt, 1], local_paths_pixels[:, opt, 0]]
                collision_free = collisions.sum() == 0

                if collision_free:
                    valid_opts.append(opt)
                    # heuristic is distance to goal
                    pts = local_paths_pixels[:, opt] # (2, N)
                    goals = (self.map_origin[:2] + self.cur_goal[..., :2]) / self.map_resolution # (2, )

                    dist_to_goal = np.linalg.norm(pts - goals, ord=2, axis=0).sum()
                    final_cost.append(dist_to_goal)

            # calculate final cost and choose best option
            # print("TO DO: Calculate the final cost and choose the best control option!")
            final_cost = np.asarray(final_cost)
            if len(valid_opts) == 0:  # hardcoded recovery if all options have collision
                control = [-.1, 0]
            else:
