...
class PathPlanner:
    ...
    def sample_map_space(self):
        ...
        # Map shape is stored as height, width
        sample = np.random.randint(0, [self.map_shape[1], self.map_shape[0]])
    
    def check_if_duplicate(self, point):
        #Check if point is a duplicate of an already existing node
        return np.any(np.all(np.equal(point, [node.point[:2] for node in self.nodes]), axis=1))
    ...
    def point_to_cell(self, point):
        ...
        # subtract by offset given in yaml
        x,y = point[:2] - self.map_settings_dict["origin"][:2]
        # divide by resolution to convert to pixel
        height = self.map_shape[1]*self.map_settings_dict["resolution"]
        resolution = self.map_settings_dict["resolution"]
        x_pix = (x/resolution).astype(int)
        y_pix = ((height-y)/resolution).astype(int)
        map_idx = np.vstack((x_pix,y_pix))
        return map_idx

    def points_to_robot_circle(self, points):
        #Convert a series of [x,y] points to robot map footprints for collision detection
        #Hint: The disk function is included to help you with this function
        robot_occupancy_map = np.zeros(self.map_shape)
        for point in points:
        	# convert point from metric to map coordinates
        	point_map = self.point_to_cell(point)
        	# resolution is metres/cell
        	robot_occupancy_map = robot_occupancy_map + disk(point_map, self.robot_robot/self.map_settings_dict['resolution'], robot_occupancy_map.shape)
        # convert to boolean
        robot_occupancy_map = robot_occupancy_map > 0
        return robot_occupancy_map
