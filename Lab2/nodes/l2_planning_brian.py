...
class PathPlanner:
    ...
    def points_to_robot_circle(self, points):
        ...
        for i in range(point_map_idx.shape[1]):
            ...
            rr = np.clip(rr, 0, self.map_shape[1] - 1) # x
            cc = np.clip(cc, 0, self.map_shape[0] - 1) # y
            #print(rr, cc)
            footprints.append(np.stack((rr, cc), axis=-1))
        return np.stack(footprints, axis=0) #(N, point_per_circle, 2) xy
